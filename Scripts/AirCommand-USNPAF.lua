...
local function updateTrack(trackID, target)
	if tracks[trackID].lastUpdate == nil or timer.getTime() > tracks[trackID].lastUpdate + 1 then
		if tracks[trackID].x == nil or tracks[trackID].y == nil then
			...
			env.info("USN/PAF Debug: Track ID " .. tostring(trackID) .. " heading: " .. tracks[trackID].heading)
			env.info("USN/PAF Debug: Track ID " .. tostring(trackID) .. " velocity: " .. tracks[trackID].velocity .. "m/s")
		end
	end
...
local function createTrack(target)
	...
	env.info("USN/PAF Debug: Created new track ID " .. tostring(nextTrackNumber) .. ". Category: " .. tracks[nextTrackNumber].category)
	env.info("USN/PAF Debug: Updated track ID " .. tostring(nextTrackNumber) .. " with target " .. target.object:getTypeName() .. " " .. tostring(target.object:getID()), 0)
...
local function timeoutTracks()
	for trackID, track in pairs(tracks) do
		if track.lastUpdate < (timer.getTime() - trackTimeout) then
			tracks[trackID] = nil
			env.info("USN/PAF Debug: Timed out lost track ID " .. tostring(trackID) .. " after " .. tostring(timer.getTime() - track.lastUpdate) .. " seconds", 0)
		end
	end
...
local function controlIntercept(flightData)
	...
	if flightData.tactic == interceptTactic.Stern or flightData.tactic == interceptTactic.Beam then
		env.info("USN/PAF Debug: Flight " .. tostring(flightGroup:getID()) .. " intercepting " .. tostring(interceptTarget) .. " attempting conversion", 0)
		convert = true
	end
	...
	if convert ~= true then
		...
		elseif distance < 5000 and math.abs(tracks[interceptTarget].alt - getLowestFlightAltitude(flightGroup)) > 5000 then
			targetDetected = true
			env.info("USN/PAF Debug: Flight " .. tostring(flightGroup:getID()) .. " intercepting " .. tostring(interceptTarget) .. " target detected by altitude exception", 0)
		end
	end
			...
			if flightData.flightGroup:isExist() then
				...
				if flightData.interceptTarget ~= nil then
					...
					if tracks[flightData.interceptTarget] == nil then
						...
						env.info("USN/PAF Debug: Flight " .. tostring(flightID) .. " tactic: " .. tostring(packages[packageID].flights[flightID].tactic), 0)
